...
def render_plantpal_page():
    ...
    # ── Page Header ──
    st.markdown("""
    <div style="background:linear-gradient(135deg,#1B5E20,#2E7D32,#43A047);padding:52px 60px 40px;color:white;position:relative;overflow:hidden;">
        ...
        <div style="position:relative;z-index:2;">
            <div style="display:inline-block;background:rgba(255,255,255,0.15);border:1px solid rgba(255,255,255,0.3);border-radius:20px;padding:5px 14px;font-size:11px;font-weight:600;letter-spacing:2px;text-transform:uppercase;margin-bottom:14px;">AI-Powered</div>
            <h1 style="font-family:'Playfair Display',serif;font-size:42px;font-weight:700;margin:0 0 10px;line-height:1.2;">PlantPal <span style="font-style:italic;color:#A5D6A7;">Chatbot</span></h1>
            <p style="font-size:16px;opacity:0.85;max-width:560px;line-height:1.6;margin:0;">
                Upload a photo of your plant and our AI will diagnose its health, 
                recommend Verdant products, and create a personalised care schedule for you.
            </p>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    # ── Layout: left = chat, right = product catalogue ──
    chat_col, prod_col = st.columns([3, 2], gap="large")

    with chat_col:
        st.markdown("""
        <div style="background:white;border-radius:16px;border:1px solid #E8E8E0;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.07);">
            <div style="background:linear-gradient(135deg,#1B5E20,#2E7D32);padding:16px 20px;display:flex;align-items:center;gap:12px;">
                ...
                <div>
                    <div style="font-family:'Playfair Display',serif;font-size:16px;font-weight:700;color:white;">PlantPal</div>
                    <div style="font-size:11px;color:rgba(255,255,255,0.75);">Upload a plant photo to get started</div>
                </div>
                <div style="margin-left:auto;width:9px;height:9px;border-radius:50%;background:#69F0AE;box-shadow:0 0 6px #69F0AE;"></div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with chat_container:
            if not st.session_state.plantpal_messages:
                st.markdown("""
                <div style="background:#F1F8F1;border-radius:12px;padding:18px 20px;border-left:3px solid #2E7D32;margin:16px 0 8px;">
                    <div style="font-size:14px;font-weight:600;color:#1B5E20;margin-bottom:6px;">👋 Hello! I'm PlantPal</div>
                    <p style="font-size:13px;color:#555;margin:0;line-height:1.6;">
                        I can help you diagnose plant health issues, recommend care products from our store, 
                        and build a personalised watering & fertilizer schedule.<br><br>
                        <strong>Upload a photo of your plant below</strong> — or just describe what you're seeing!
                    </p>
                </div>
                """, unsafe_allow_html=True)
            else:
                for msg in st.session_state.plantpal_messages:
                    if msg["role"] == "user":
                        st.markdown(f"""
                        <div style="display:flex;justify-content:flex-end;margin:8px 0;">
                            <div style="background:#2E7D32;color:white;border-radius:16px 16px 4px 16px;padding:12px 16px;max-width:85%;font-size:13px;line-height:1.5;">
                                {msg["content"]}
                            </div>
                        </div>
                        """, unsafe_allow_html=True)
                        ...
                        <div style="display:flex;justify-content:flex-start;margin:8px 0;">
                            <div style="background:#F8FAF8;border:1px solid #C8E6C9;border-radius:16px 16px 16px 4px;padding:12px 16px;max-width:92%;font-size:13px;line-height:1.6;color:#2A2A2A;">
                                {msg["content"]}
                            </div>
                        </div>
                        """, unsafe_allow_html=True)
        ...
        uploaded_file = st.file_uploader(
            "📷 Upload plant photo (JPG/PNG)",
            type=["jpg", "jpeg", "png", "webp"],
            key="plantpal_photo_upload",
            help="Upload a photo of your plant for AI diagnosis"
        )

        if uploaded_file:
            st.image(uploaded_file, caption="Your plant", use_container_width=True)

        # ── Text input ──
        user_text = st.text_area(
            "Or describe what you're seeing…",
            placeholder="e.g. 'My monstera has yellow leaves and dry soil' or ask anything about plant care",
            key="plantpal_text_input",
            height=80,
            label_visibility="collapsed"
        )
        ...
        with send_col:
            send_clicked = st.button("✉️ Send to PlantPal", key="plantpal_send", type="primary", use_container_width=True)
        with clear_col:
            if st.button("🗑️ Clear", key="plantpal_clear", use_container_width=True):
                st.session_state.plantpal_messages = []
                st.session_state.plantpal_uploaded_image = None
                st.rerun()

        if send_clicked and (uploaded_file or user_text):
            # Build message content for Claude
            # Prepare the product catalogue summary for context
            product_summary = "\n".join([
                f"- ID {p['id']}: {p['name']} | Category: {p['category']} | Price: ₹{p['price']} | {p['description'][:80]}"
                for p in PRODUCTS if p["category"] in [
                    "Fertilizers", "Pest Control", "Plant Medicines", "Soil & Compost",
                    "Pots & Planters", "Watering Accessories", "Gardening Tools",
                    "Indoor Plants", "Outdoor Plants", "Air Purifying Plants", "Succulents",
                    "Bonsai Plants", "Flowering Plants"
                ]
            ])

            system_prompt = f"""You are PlantPal, a friendly and knowledgeable AI plant care assistant for Verdant Premium Plant Boutique (an Indian plant store). 

Your job is to:
1. Diagnose plant health issues from photos or descriptions
2. Recommend specific products FROM OUR STORE (use the product list below)
3. Create a clear care schedule (e.g., "Monday & Thursday: Apply X fertilizer")
4. Be warm, encouraging, and concise

Always recommend products by their exact name from our catalogue. Format product recommendations as:
**Recommended: [Product Name]** (₹[price]) — [brief reason]

For schedules, use a simple weekly table format with emojis.

VERDANT PRODUCT CATALOGUE:
{product_summary}

Rules:
- Only suggest products from the catalogue above
- Always give a weekly care schedule when suggesting fertilizers/treatments
...
- End with an encouraging note 🌱"""
            ...
            text_query = user_text if user_text else "Please diagnose this plant from the photo and recommend products."
            content_parts.append({"type": "text", "text": text_query})

            # Show user message
            display_text = user_text if user_text else "📷 [Plant photo uploaded]"
            st.session_state.plantpal_messages.append({"role": "user", "content": display_text})

            with st.spinner("🌿 PlantPal is analysing your plant…"):
                try:
                    ...
                    mentioned_products = []
                    for p in PRODUCTS:
                        if p["name"].lower() in ai_response.lower():
                            mentioned_products.append(p)

                    st.session_state.plantpal_messages.append({
                        "role": "assistant",
                        "content": ai_response.replace("\n", "<br>").replace("**", "<strong>", 1)
                    })
                    st.session_state.plantpal_mentioned_products = mentioned_products

                except Exception as e:
                    st.session_state.plantpal_messages.append({
                        "role": "assistant",
                        "content": f"Sorry, I had trouble connecting. Please try again. ({str(e)[:80]})"
                    })
    ...
    with prod_col:
        st.markdown("""
        <div style="font-family:'Playfair Display',serif;font-size:20px;font-weight:700;color:#1B5E20;margin-bottom:4px;">Recommended Products</div>
        <p style="font-size:13px;color:#6B6B6B;margin-bottom:16px;">Products PlantPal suggests for your plant</p>
        """, unsafe_allow_html=True)

        # Show AI-recommended products if available
        mentioned = getattr(st.session_state, "plantpal_mentioned_products", [])

        if mentioned:
            for product in mentioned[:4]:
                discount = int((1 - product["price"] / product["original_price"]) * 100)
                st.markdown(f"""
                <div style="background:white;border-radius:12px;border:1px solid #C8E6C9;padding:14px;margin-bottom:12px;box-shadow:0 2px 8px rgba(0,0,0,0.06);">
                    <div style="display:flex;gap:12px;align-items:flex-start;">
                        <img src="{product['image']}" style="width:70px;height:70px;border-radius:8px;object-fit:cover;flex-shrink:0;" />
                        <div style="flex:1;">
                            <div style="font-size:13px;font-weight:700;color:#2A2A2A;margin-bottom:2px;">{product['name']}</div>
                            <div style="font-size:11px;color:#6B6B6B;line-height:1.4;margin-bottom:6px;">{product['description'][:70]}…</div>
                            <div style="display:flex;align-items:center;gap:8px;">
                                <span style="font-size:14px;font-weight:700;color:#2E7D32;">₹{product['price']:,}</span>
                                <span style="font-size:11px;color:#E53935;background:#FFF0F0;padding:1px 6px;border-radius:4px;">-{discount}%</span>
                            </div>
                        </div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
                if st.button(f"Add to Cart — {product['name'][:20]}", key=f"pp_cart_{product['id']}", use_container_width=True, type="primary"):
                    add_to_cart(product["id"])
                    st.success(f"'{product['name']}' added! 🛒")
                    st.rerun()
        else:
            # Show default care products as suggestions
            default_care = [24, 27, 26, 30]  # NPK Fertilizer, Neem Oil, Potting Mix, Seaweed Tonic
            st.markdown("""
            <div style="background:#F1F8F1;border-radius:10px;padding:14px;border:1px dashed #A5D6A7;margin-bottom:16px;">
                <div style="font-size:12px;color:#2E7D32;font-weight:600;">💡 Upload a plant photo</div>
                <div style="font-size:12px;color:#6B6B6B;margin-top:4px;">PlantPal will recommend the right products for your specific plant's needs.</div>
            </div>
            """, unsafe_allow_html=True)
            st.markdown("<div style='font-size:12px;font-weight:600;color:#6B6B6B;letter-spacing:1px;text-transform:uppercase;margin-bottom:12px;'>Popular Care Products</div>", unsafe_allow_html=True)
            for pid in default_care:
                product = get_product_by_id(pid)
                if not product:
                    continue
                discount = int((1 - product["price"] / product["original_price"]) * 100)
                st.markdown(f"""
                <div style="background:white;border-radius:12px;border:1px solid #E8E8E0;padding:12px;margin-bottom:10px;">
                    <div style="display:flex;gap:10px;align-items:center;">
                        <img src="{product['image']}" style="width:56px;height:56px;border-radius:8px;object-fit:cover;" />
                        <div>
                            <div style="font-size:12px;font-weight:700;color:#2A2A2A;">{product['name']}</div>
                            <div style="font-size:11px;color:#6B6B6B;margin:2px 0;">{product['description'][:55]}…</div>
                            <span style="font-size:13px;font-weight:700;color:#2E7D32;">₹{product['price']:,}</span>
                            <span style="font-size:10px;color:#E53935;margin-left:4px;">-{discount}%</span>
                        </div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
                if st.button(f"Add — {product['name'][:18]}", key=f"pp_def_{product['id']}", use_container_width=True):
                    add_to_cart(product["id"])
                    st.success("Added! 🛒")
                    st.rerun()
